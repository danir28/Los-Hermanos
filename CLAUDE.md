...
### Los tests de `orders/service.test.ts` empezaron a fallar según la hora del día, por el horario comercial de la base de test

Al extender el bloqueo por horario cerrado a los tres canales (cliente, recepción, cocina — ver "Bloqueo agregar productos..." más arriba, 12/8/2026), `createOrder()` pasó a llamar `isBusinessOpenNow()` sin importar `type`, no solo para `"online"`. Corriendo `npm test --prefix server` de noche (22:18), 8 tests de `orders/service.test.ts` empezaron a fallar con `BusinessClosedError: La rotisería está cerrada en este momento` — la tabla `business_hours` de `los_hermanos_test` tenía cargado el horario real del local (ej. miércoles 10:00–21:00), así que cualquier test que llamara a `createOrder()` pasaba o fallaba según la hora real en que corriera la suite, no según el código. Se resolvió actualizando `business_hours` en `los_hermanos_test` (nunca en `los_hermanos`, la de dev, que sí debe reflejar el horario real) para que quede abierta las 24 horas los 7 días: `UPDATE business_hours SET "isOpen" = true, ranges = '[{"openTime":"00:00","closeTime":"00:00"}]';` — mismo openTime/closeTime aprovecha a propósito la lógica de "cruza medianoche" que ya tiene `isWithinRange` (`businessHours/service.ts`) para cubrir el día completo sin ningún hueco, en vez de dejar un minuto sin cubrir con `"23:59"`. Moraleja: los datos de una base de test nunca deberían hacer que un test dependa del reloj real (misma categoría de bug que ya evitan `usableSlotForToday()`/`describe.skipIf` en ese mismo archivo, pero acá el horario comercial se había colado sin ese cuidado) — cualquier tabla de "horario" en la base de test debe quedar sin restricción, salvo que el test sea específicamente sobre esa restricción.
