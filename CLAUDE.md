...
**Known interim gap:** `GET /api/orders/lookup?orderNumber=` (public, used by the customer's tracking screen) can no longer `findUnique` by `orderNumber` alone, since it can repeat across business days. It currently does `findFirst` ordered by `businessDate desc` (most recent match wins) as a stopgap — this is intentionally not yet restricted to "only today's business day", which is planned as separate follow-up work on the `cliente` branch. Until that lands, searching by an old order number could, in principle, resolve to a different (also old) order with the same number from a different day rather than correctly failing. This is deliberate, sequenced work — see the plan discussion in that branch when it's tackled, including handling the same 7am-boundary edge case (an order search after midnight but before 7am must still resolve against the previous business day, not the new calendar date).
